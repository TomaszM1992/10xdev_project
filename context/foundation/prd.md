...
Existing productivity apps expose a priority feature but ignore time. They have no answer to "I have 90 minutes before my next meeting — what from my list fits and matters most?" The user ends up doing that mental math unaided, or skipping it entirely and dismissing tasks. The insight: surfacing the priority × time fit for a given day is what turns a backlog browser into a daily action engine.
...
- The daily view must be immediately usable on open — the task list renders without perceptible delay for a typical task backlog.
...
- **Then** they see tasks for today (and any overdue carried-over tasks) ordered by priority × time fit within their declared available hours, with overdue tasks visually distinct from same-day tasks
...
- Tasks from past dates that were neither completed nor dismissed appear at the top, visually distinct from same-day tasks
- Each task displays its name, priority, time estimate, and status
- The user can see and adjust their declared available hours directly on the daily view
- The task list renders without perceptible delay on open
...
### Daily View
- FR-011: User can view all tasks for a given date, ordered by a combination of priority and time estimate within the user's declared available hours. Priority: must-have
...
- The daily view is immediately usable on open — the task list renders before the user perceives a delay (target: under 200ms for a typical backlog of up to 200 tasks).
...
- The user can declare and adjust their available hours for the day at any time directly on the daily view.
...
The user opens the daily view and immediately gets a prioritized, time-feasible list of tasks to work through. They do not need to manually judge "does this fit in my day?" — the app surfaces the answer.

## Access Control

Single user, email and password login. Flat user model — no roles, no sharing, no team workspaces. A logged-in user has full access to all their own tasks and settings. An unauthenticated user is redirected to the login screen; no public or guest access is provided.
...
- **No external identity providers**: email + password only for v1. Social login via third-party identity services deferred until after core habit formation.
- **No external calendar integration**: import or sync from external calendar services is explicitly out of scope; would require complex integration requirements before the core product is proven.
...
- **No native mobile app**: responsive web only; native mobile explicitly deferred.
